...
func (b *Breaker) Acquire(ctx context.Context) (*Permit, error) {
	...
	for {
		b.mu.Lock()
		permit, err, wait := b.tryAcquireLocked()
		if !wait {
...
func (b *Breaker) tryAcquireLocked() (*Permit, error, bool) {
...
func (p *Permit) Complete(outcome Outcome, slow bool) error {
	...
	if b.state == StateHalfOpen {
		b.completeHalfOpenLocked(outcome, now)
		return nil
	}
	...
	if openingDecision(b.config.opening, b.config.minimumThroughput, b.consecutiveFailures, snapshot) {
		b.openLocked(now, ReasonPolicyOpened)
	}
	return nil
}
...
func (p *Permit) Cancel() error {
	...
	if !b.config.clock.Now().Before(p.deadline) {
		p.expireLocked()
		return ErrPermitExpired
	}
...
func (b *Breaker) completeHalfOpenLocked(outcome Outcome, now time.Time) {
	b.halfOpenActive--
	b.signalLocked()
	if outcome == OutcomeIgnored {
		return
	...
	if outcome == OutcomeSuccess {
		...
		b.openLocked(now, ReasonHalfOpenFailed)
		return
	}
...
func (b *Breaker) openLocked(now time.Time, reason TransitionReason) {
	b.transitionLocked(StateOpen, reason, now, func() {
		b.openCount++
		b.currentOpenDuration = b.openDurationLocked()
...
func (b *Breaker) Reset() error {
	b.mu.Lock()
	defer b.unlockAndDispatch()
	b.transitionLocked(StateClosed, ReasonReset, b.config.clock.Now(), func() {
		b.window = newRollingWindow(b.config)
		b.mode = ModeNormal
		b.consecutiveFailures = 0
		b.openCount = 0
...
func (b *Breaker) openDurationLocked() time.Duration {
	...
	return b.jitterDurationLocked(duration)
}

func (b *Breaker) jitterDurationLocked(duration time.Duration) time.Duration {
	if b.config.openDurationJitter == 0 {
		return duration
	}
	sample := safeRandomSample(b.config.random)
