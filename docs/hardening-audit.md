...
| integrations | executable HTTP ordering/body/retry harness; nested `database/sql` and published `go-jsonrpc` client suites; Valkey, queue, and storage classifier contracts |
...
an adapter. On 2026-07-16, the now-concrete `go-http-client` implementation was
...
`go-jsonrpc` suite pins the published client, separates local method validation
