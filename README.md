...
`go-circuit-breaker` is a protocol-neutral, bounded, concurrent circuit breaker
...
generation, so stale permits cannot mutate the current state.
...
- Use `Snapshot` for bounded aggregate state. It never contains results or
  operation errors.
- Use an `Observer` for transition events. Asynchronous delivery is bounded and
  must be closed by its owner with `Breaker.Close`.
