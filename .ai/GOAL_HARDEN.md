# Goal Harden: `go-circuit-breaker`
...
`go-circuit-breaker`, then implement every justified correction required for
...
- circuit-breaker state-machine literature and documented behavior from mature
  ...
  semantics
- integration contracts from `go-http-client` and representative RPC,
...
- `go-http-client` cache, limiter, breaker, retry, authentication, signing,
