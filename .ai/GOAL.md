...
These boundaries allow `go-http-client`, `go-service`, and concrete vendor
...
- Optional adapters MAY integrate `log/slog`, OpenTelemetry, and `go-telemetry`.
...
`go-http-client` owns the canonical HTTP composition and status classifier.
...
- `breakerhttp`: optional HTTP adapter only if dependency direction remains
  acyclic; otherwise HTTP integration remains in `go-http-client`
...
- integration tests through `go-http-client` and representative non-HTTP calls
...
6. Integrate with `go-http-client` and representative non-HTTP clients.
...
- `go-http-client` composes the package without duplicating breaker state.
